...
alert('admin_notice.js');




function confirme_updateNotice(event){
	...
	if( confirm('êtes-vous sur de vouloir enregistrer les modifications ?') ){
		...
		if( fichier !== ''){
			...
			reader.onloadend = function(){
				xhr.open('PUT','/app_dev.php/admin/notice/update/'+target.id,true);
...
if( window.addEventListener ){
	for ( i = 0 ; i < document.getElementsByTagName('button').length ; i++ ){
		...
		if( document.getElementsByTagName('button')[i].className == 'enregistrer' ){
			...
			document.getElementsByTagName('button')[i].addEventListener('click',confirme_updateNotice,false);
	...
	for ( i = 0 ; i < document.getElementsByTagName('button').length ; i++ ){
		...
		if( document.getElementsByTagName('button')[i].className == 'enregistrer' ){
			...
			document.getElementsByTagName('button')[i].attachEvent('onclick',confirme_updateNotice);
