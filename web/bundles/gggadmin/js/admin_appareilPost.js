...
alert('admin_postApareil.js');




function confirme_postAppareil(event){
	...
	if( confirm('êtes-vous sur de vouloir enregistrer les modifications ?') ){
		...
		if( photo !== ''){
			...
			reader.onloadend = function(){
				xhr.open('POST','/app_dev.php/admin/appareil/post',true);
...
if( window.addEventListener ){
	for ( i = 0 ; i < document.getElementsByTagName('button').length ; i++ ){
		...
		if( document.getElementsByTagName('button')[i].className == 'ajouter' ){
			...
			document.getElementsByTagName('button')[i].addEventListener('click',confirme_postAppareil,false);
	...
	for ( i = 0 ; i < document.getElementsByTagName('button').length ; i++ ){
		...
		if( document.getElementsByTagName('button')[i].className == 'ajouter' ){
			...
			document.getElementsByTagName('button')[i].attachEvent('onclick',confirme_postAppareil);
